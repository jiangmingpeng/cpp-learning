...
namespace JMP
{
	...
	public://各种函数
		...
		{
			return reverse_iterator(end());//临时对象具有常性
		}
		const_reverse_iterator rbegin() const
		{
			return const_reverse_iterator(end());//没有传const迭代器会导致传参不匹配 无法进入构造函数
		}
		iterator end()
		{
			return iterator(_head);//返回的是一个临时对象
		}
		...
		{
			return reverse_iterator(begin());//这里不使用--或者++来调整偏移量 得益于迭代器里的*的调整哈哈哈
		}
